...
void play_intro(); // Plays intro song
void sticks(); // A sticks removal game
...
Game games[] = { // Here we define games. Add your own games
  ...
  {.name = "Sticks", .fn = sticks},
  {.name = "Play Song", .fn = play_intro},
...
void sticks() {
 ...
 {
  current_vals-=3;
  soft_pwm_withaudio(led_green, 100, 1000, m_green);
  switch_player();
  updated=false;
 }
 if (!digitalRead(btn_blue))
 {
  current_vals-=4;
  soft_pwm_withaudio(led_blue, 100, 1000, m_blue);
  switch_player();
  updated=false;
 }
 if (current_vals<=1)
